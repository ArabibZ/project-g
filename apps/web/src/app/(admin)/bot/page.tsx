...
import { ConfirmDialog, EmptyState, ErrorState, LoadingState, Notice, StatusPill } from "@/components/ui";
...
export default function BotPage() {
  ...
  async function load(signal?: AbortSignal) {
    try {
      const nextBot = botSchema.parse(await api("/api/bot", { signal }));
      setError("");
      setBot(nextBot);
      if (nextBot.connected) {
        const result = subscribersSchema.parse(await api("/api/bot/subscribers", { signal }));
  ...
  useEffect(() => {
    ...
    void api("/api/bot", { signal: controller.signal })
      .then((value) => botSchema.parse(value))
      .then(async (nextBot) => {
        const nextSubscribers = nextBot.connected
          ? subscribersSchema.parse(await api("/api/bot/subscribers", { signal: controller.signal })).subscribers
          : [];
        return { nextBot, nextSubscribers };
      })
      .then(({ nextBot, nextSubscribers }) => {
        setError("");
        setBot(nextBot);
  ...
  if (!bot && !error) return <LoadingState label="Loading bot" />;
  if (!bot) return <ErrorState message={error} retry={() => void load()} />;
