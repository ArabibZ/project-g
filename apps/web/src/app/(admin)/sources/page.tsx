...
export default function SourcesPage() {
  ...
  async function load(signal?: AbortSignal) {
    try {
      const result = sourcesSchema.parse(await api("/api/sources", { signal }));
  ...
  useEffect(() => {
    ...
    void api("/api/sources", { signal: controller.signal })
  ...
  if (!loaded) return <LoadingState label="Loading sources" />;
            ...
            >
              ...
              <div className="source-main">
                ...
                <div className="source-meta">
                  <span>Checked {formatRelative(source.lastCheckedAt)}</span>
                  {source.lastFailedAt ? (
                    <span className="failure-copy" title={source.lastError ?? undefined}>
                      Failed {formatRelative(source.lastFailedAt)} ({source.failureCount})
                    </span>
                  ) : (
